//*   TASK: Generate several reports for the bank executive leadership. 
...
Select [BORROWERID] = DBI.[BorrowerID],
	   ...
	   FROM [dbo].[LoanSetupInformation] AS LOS
	   RIGHT JOIN [dbo].[Borrower] AS DBI               ----<--DIFFERENCE = USE 'LEFT JOIN'
	   ON LOS.[BorrowerID] = DBI.[BorrowerID]
...
RETURN
	...
	---------- DEMO-----
	DECLARE @TotalPurchaseAmnt MONEY = 0    -----ENABLES Dynamic/Flexible query
	-----------
	SELECT *        
...
--DECLARE @MinPurchaseAmnt MONEY = 10000;
--DECLARE @MaxPurchaseAmnt MONEY = 100000;

--SELECT *
--FROM dbo.BorrowerPurchaseData_FCN()
--WHERE [TOTAL PURCHASE AMOUNT] > @MinPurchaseAmnt 
--AND [TOTAL PURCHASE AMOUNT] <= @MaxPurchaseAmnt
--ORDER BY [TOTAL PURCHASE AMOUNT] DESC;
...
SELECT [YEAR OF PURCHASE] = YEAR (LSA.[PurchaseDate]), 
	   ...
	   [AVERAGE PURCHASE AMOUNT] = FORMAT (AVG (LSA.[PurchaseAmount]), 'C0'),
	   [COUNT OF BORROWERS] = COUNT (LSA.[BorrowerID]),
	   [AVERAGE LTV] = FORMAT (AVG (LSA.[LTV]), 'P2'),
	   [MINIMUM LTV] = FORMAT (MIN (LSA.[LTV]), 'P2'),
	   [MAXIMUM LTV] = FORMAT (MAX (LSA.[LTV]), 'P2'),
	   [AVERAGE AGE OF THE BORROWERS] =  AVG (FLOOR (DATEDIFF(day, DBA.[DoB], GETDATE()))/365)
...
GROUP BY [YEARS LEFT TO MATURITY (BINS)]  ---Non-aggregated Column
