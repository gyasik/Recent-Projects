...
CREATE FUNCTION dbo.BorrowerPurchaseData_FCN       ----CREATE FUNCTION
...
RETURN
	...
	---------- DEMO-----
	DECLARE @PurchaseAmount MONEY = 0    -----ENABLES Dynamic query
	   SELECT *        
	   FROM dbo.BorrowerPurchaseData_FCN (@PurchaseAmount)
	   WHERE [TOTAL PURCHASE AMOUNT] > @PurchaseAmount
	   ORDER BY  [TOTAL PURCHASE AMOUNT] DESC
